...
with col_b1:
    st.subheader(f"🏏 {st.session_state.striker} *")
    st.write(f"**{s_p['runs']}** ({s_p['balls']}) | 4s: {s_p['4s']} | 6s: {s_p['6s']} | SR: {s_sr:.1f}")

with col_b2:
    st.subheader(f"🏏 {st.session_state.non_striker}")
    st.write(f"**{ns_p['runs']}** ({ns_p['balls']}) | 4s: {ns_p['4s']} | 6s: {ns_p['6s']} | SR: {ns_sr:.1f}")

st.write(f"⚾ **Bowler: {st.session_state.current_bowler}** — Overs: {bw_ov} | Wkts: {bw_p['wickets']} | Runs: {bw_p['b_runs']} | Econ: {bw_ec:.2f}")
...
with st.expander("📊 Full Match Scorecard & Performance Analytics"):
    ...
    with col_mvp:
        ...
        if st.button("Calculate MVP Player"):
            ...
            if best_player:
                st.success(f"🌟 **MVP: {best_player}** ({highest_pts:.0f} pts)")
    ...
    with col_gc:
        ...
        if st.button("Find Game Changer"):
            ...
            if gc_type == "Batting":
                ...
                for p, s in st.session_state.player_stats.items():
                    if s["balls"] >= 3:
                        ...
                        # Weighted impact score combining runs and strike rate
                ...
                for p, s in st.session_state.player_stats.items():
                    if s["b_balls"] > 0:
                        econ = (s["b_runs"] / (s["b_balls"] / 6)) if s["b_balls"] > 0 else 0.0
                        # Wickets heavily weighted, low economy rewarded
            ...
            if gc_player:
                st.success(f"⚡ **{gc_type} Game Changer: {gc_player}**")
    ...
    def generate_pdf():
        ...
        summary_data = [
            ["Innings", "Runs", "Wickets", "Overs", "DRS Remaining"],
