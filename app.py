...
if "initialized" not in st.session_state:
    st.session_state.total_players_combined = 16
...
with st.expander("⚙️ Match Setup, Captains & Squads"):
    ...
    batting_squad = list(base_bat)
    if st.session_state.has_joker and st.session_state.joker_name not in batting_squad:
        batting_squad.append(st.session_state.joker_name)

    bowling_squad = list(base_bowl)
    if st.session_state.has_joker and st.session_state.joker_name not in bowling_squad:
        bowling_squad.append(st.session_state.joker_name)
